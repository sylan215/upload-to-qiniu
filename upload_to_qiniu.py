...
access_key = '你的AccessKey' # 个人中心->密匙管理->AK
secret_key = '你的SecretKey' # 个人中心->密匙管理->SK

bucket_name = '你的存储空间名' # 七牛空间名
...
def upload_qiniu(input_path):
    ...
    if ret and ret['key'] == key:
        print('%s done' % ('http://github.com/' + dir_set + filename))
    else:
        print('%s error' % ('http://github.com/' + dir_set + filename))
